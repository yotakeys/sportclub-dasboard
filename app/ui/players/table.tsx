...
export default function PlayersTable({ players, region }: { players: PlayerWithGroups[]; region?: string }) {
  const [selectedPlayer, setSelectedPlayer] = useState<PlayerWithGroups | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const handleRowClick = (player: PlayerWithGroups) => {
            ...
            {players.length === 0 ? (
              ...
              players.map((player) => (
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-right text-sm" onClick={handleActionClick}>
                    <div className="flex justify-end gap-2">
                      <UpdatePlayer id={player.id} region={region} />
...
function UpdatePlayer({ id, region }: { id: string; region?: string }) {
  return (
    <Link
      href={`/dashboard/players/${id}/edit?region=${region || 'Surabaya'}`}
